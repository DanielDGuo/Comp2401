...
int concatStrings(char *s1, char *s2, char *outString, int sizeOfOutstring)

{
	if(sizeOfOutstring>=requiredStrLength(s1, s2)){
		for(int i = 0; i < strlen(s1); i++){
			outString[i]=s1[i];
		}
		for(int i = 0; i < strlen(s2); i++){
			outString[strlen(s1)+i] = s2[i];
		}
		outString[strlen(s1)+strlen(s2)] = '\0';
		return(0);
	}else{
		return(1);
	}
